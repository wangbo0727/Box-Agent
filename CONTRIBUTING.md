...
- Model calls go through the run-scoped `tools/engine/DefaultToolEngine`; standalone calls use `runtime.invoke_tool_with_permissions`. The engine owns preparation, execution, permission continuation and result adaptation, while `kernel/tool_messages.py` commits the final reply. Preserve the caller's Tool instances and session resources. See the [Tool Engine implementation guide](docs/design/tool-refactor/runtime.md).
