...
| C5 | 两档暴露、本地/MCP 发现、规范名迁移、全入口收口 | 已实现并做定向验证；必要工具直接提供，低频工具可发现；定时任务新规范名与旧名兼容。真实宿主尚未完成运行验证 |
| C6 | 全回归、ACP 真实任务与产物、打包安装探针、实际宿主、最终 Head 门禁与 PR | 进行中；等待固定集成 Head 的全量 preflight、新 wheel 探针、PR 真实任务对照与正式审查 |

## 验收维度

| 维度 | 所需证据 | 状态 |
| --- | --- | --- |
| 装配与能力 | 各支持宿主/配置/子 Agent；canonical/alias/schema；eager/deferred 覆盖、profile | C1 固定夹具 + C5 明确差异夹具；定向用例通过，最终全量待验证 |
...
| 端到端 | 固定模型/Skill/配置/预算/输入的 ACP 基线与新实现；文件、复杂搜索/委派、PPT 产物 | d2 基线完成，PR 对照待跑；latest main PPT 对照另补 |
| 收益 | 任务成功、误选、参数错误、发现轮数、schema/总 token、缓存、延迟 | 待 PR 固定版本真实任务完成；不以少几个常驻 schema 直接宣称整体收益 |
| 工程 | 新进程 import、完整 preflight、wheel 安装/探针、准确最终 Head、只一个 Tool PR | 评测器 113 passed；源码开发期全量曾有 6 个失败，均已修复并以 36 个定向用例复核；尚待最终 Head 全量结果 |
...
完整输入、输出、不可变 attempt、渲染、诊断和脱敏清单保存在本地 TestRun/tool-refactor-20260909，未纳入 Git。基线失败不自动使新版本通过；PR 必须独立运行并检查实际产物。
