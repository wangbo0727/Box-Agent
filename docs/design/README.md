...
| Skill discovery, `get_skill` / `list_skills`, explicit selection, Skill context budgets and recovery | [Skill Engine 实现说明](skill-engine.md), then [Session and Skill ownership](../WORKFLOW_OWNERSHIP.md) | Skill owns valid immutable source snapshots, selection and read facts; Context owns current coverage, ordinary reference projection and paging budgets. Composition binds run-scoped Context to the final plugin Skill/Store. Native hash snapshots and bounded inline references use the existing `request/context` event without requiring new Store methods. Real reads remain tool messages; child scope and Tool permissions stay enforced. | `tests/test_skill_catalog_tool.py`, `test_context_input.py`, `test_skill_context.py`, `test_skill_context_regressions.py`, `test_skill_reference_persistence.py`, `test_agent_session_persistence.py`, `test_skill_plugin_composition.py`, `test_acp.py`, and child delegation tests |
