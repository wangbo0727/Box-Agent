...
| Skill discovery, `get_skill` / `list_skills`, explicit selection, Skill context budgets and recovery | [Skill Engine 实现说明](skill-engine.md), then [Session and Skill ownership](../WORKFLOW_OWNERSHIP.md) | Catalog metadata and method bodies are separate. Real reads stay in tool messages; host-selected references use bounded user request copies. Session facts do not imply current visibility. Source/hash recovery, immutable snapshots, child assignment scope and final plugin reader/catalog binding are validated without promoting bodies into system. | `tests/test_skill_catalog_tool.py`, `test_skill_context.py`, `test_skill_context_regressions.py`, `test_skill_reference_persistence.py`, `test_agent_session_persistence.py`, `test_skill_plugin_composition.py`, `test_acp.py`, and child delegation tests |
...
- Skill bodies are ordinary references; host selection, actual reads, request
  visibility and compatible billing attribution remain separate facts;
