...
- **行为变化：** 取消固定名称/关键词驱动的自动全文及 system 激活；新增完整本地目录分页工具 `list_skills`，真实 `get_skill` / `skill_view` 返回 tool 正文。显式选择以普通资料持续投影到当前 user 请求副本，原始输入不改写。多选整组正文放不下时改走名称提示与 `get_skill` 分页，按下一完整请求、同批共享额度及 UTF-8 成本核算预算。宿主资料按追加后实际 Message 的转义与包装增量核算预算；连显式选择分页提示都放不下时，Context 返回阻断原因，Kernel 在 provider 前以 Error/Done 结束。
