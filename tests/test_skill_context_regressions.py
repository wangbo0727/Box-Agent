...
from box_agent.skill_context import read_reference, render_reference
...
def make_runtime(tmp_path):
    ...
    return SkillRuntime(loader), settings
...
def added_text(context, user_index=0):
    content = context.messages[user_index].content
...
def test_transitive_required_invalidation_retires_previously_read_parent(tmp_path, invalidation):
    ...
    runtime, settings = make_runtime(tmp_path)
    messages = [Message(role="user", content="task")]
    runtime.prepare_context(messages, budget_chars=50000)
    first = runtime.read("parent")
    ...
    assert "parent" not in runtime.active_names
    status = "\n".join(context.diagnostics)
    assert "parent" in status and "leaf" in status and "historical" in status
    assert "historical" in added_text(context)
    reread = runtime.read("parent")
...
def test_selected_reference_with_budget_smaller_than_host_prefix_stays_bounded(tmp_path):
    ...
    runtime.select(["small", "missing"])
    messages = [Message(role="system", content="BASE"), Message(role="user", content="task")]

    context = runtime.prepare_context(messages, budget_chars=20)

    assert len(added_text(context, 1)) <= 20
    assert not context.references
    assert "SMALL_METHOD" not in added_text(context, 1)
    assert messages[1].content == "task"
    assert context.messages[0].content == "BASE"
...
def test_oversized_selection_keeps_all_selected_skills_readable_without_pinning_short_body(tmp_path):
    ...
    runtime, _ = make_runtime(tmp_path)
    runtime.select(["long", "short"])
    messages = [Message(role="user", content="task")]

    context = runtime.prepare_context(messages, budget_chars=2000)

    assert "SHORT_METHOD" not in added_text(context)
    assert "LONG_METHOD_" not in added_text(context)
    assert not context.references
    assert any("long" in diagnostic and "paged" in diagnostic for diagnostic in context.diagnostics)
    assert "short" in added_text(context) and "get_skill" in added_text(context)
    assert runtime.log_records() == []
    assert "SHORT_METHOD" in runtime.read("short").model_context
...
def test_small_selection_leaves_large_selection_a_path_to_later_pages(tmp_path):
    ...
    for index in range(100):
        context = runtime.prepare_context(messages, budget_chars=2000)
        page = runtime.read("long", offset=offset, revision=revision)
        assert page.success, (
            f"The selected large Skill cannot advance from line {offset}: {page.error}; "
            f"host projection already spent {len(added_text(context))}/2000 characters"
        )
...
def test_reuse_receipt_also_respects_remaining_reference_budget(tmp_path):
    ...
    original = runtime.read("demo")
    runtime.prepare_context([tool_message(original, "visible-body")], budget_chars=20)

    receipt = runtime.read("demo")
...
def test_selected_same_source_update_and_disable_use_current_status(tmp_path):
    ...
    updated = runtime.prepare_context(messages, budget_chars=50000)
    ...
    assert runtime.log_records()[0]["sha256"] == updated.references[0]["revision"]
    settings.write_text('{"disabledSkillNames":["demo"]}', encoding="utf-8")
    disabled = runtime.prepare_context(messages, budget_chars=50000)
    ...
    assert "demo" not in runtime.active_names
    assert not runtime.read("demo").success
...
def test_completed_pages_replaced_by_summary_do_not_count_as_visible_body(tmp_path, summary_format):
    ...
    revision = None
    for index in range(100):
        runtime.prepare_context(messages, budget_chars=2000)
        page = runtime.read("paged", offset=offset, revision=revision)
        assert page.success
        metadata = page.raw_output["skill_reference"]
    ...
    runtime.prepare_context(messages, budget_chars=2000)
    assert runtime.read("paged").raw_output["skill_reference"]["reused"]
    ...
    runtime.prepare_context(compacted, budget_chars=2000)
    reread = runtime.read("paged")
...
def test_shared_loader_does_not_share_read_coverage_or_budget(tmp_path):
    ...
    first, _ = make_runtime(tmp_path)
    second = SkillRuntime(first.loader)
    first.prepare_context([], budget_chars=50000)
    complete = first.read("demo")
    assert complete.success and complete.raw_output["skill_reference"]["complete"]
    first.prepare_context([tool_message(complete, "first-session")], budget_chars=50000)
    assert first.read("demo").raw_output["skill_reference"]["reused"]
    first.prepare_context([], budget_chars=20)
    assert not first.read("demo").success
    assert second.log_records() == []
    assert second.active_names == ()
    assert second.turn_deliveries == {}

    second.prepare_context([Message(role="user", content="separate task")], budget_chars=2000)
    ...
    assert first.log_records()[0]["deliveredComplete"]
    assert not second.log_records()[0]["deliveredComplete"]
