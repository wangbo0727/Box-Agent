...
Box-Agent 静态新建遵守根 `SKILL.md` 的“Box-Agent 静态新建的执行方式”：设计分组保留，但不决定必须启动多少个子任务；规划交付设计决策，实际像素级布局在制作时完成，不重复生成预演报告。以下事实、素材、屏显文案与质量合同不变，其他环境及已有编辑沿用原流程。
...
- `bookends` 通常包含封面与结束页；
- `dividers` 包含全部章节过渡页，使用同一个设计组维持章节系统；实际委派时由同一个 Slide 负责；
- 内容页按连续论证、共享素材或图形语言分组，但一个组还必须共享相容的制作任务。普通 cards/list 可以形成亲缘组；复杂 Canvas、独立数据图、重图像合成等若没有真正共享的构图系统，应各自成组。不要用“都是内容页”替代制作负荷判断。Box-Agent 静态新建可由主 Agent 连续制作多个设计组；实际委派时仍在一次任务内写完所属组全部首稿，由父级批量渲染、逐页看图，所有子写者返回后主 Agent 集中修复。其他环境及已有编辑保留原组分工；具备子内渲染与看图能力的环境保留 Slide 的逐页闭环。所有路径都覆盖每页像素验收和组内总览；
- 每页只能属于一个组；全部页面必须被覆盖；
- 每组写 `boundary_handoff`：至少给出进入前一页、组内首尾页和离开后一页的画布家族、明度、色场/图片处理与应延续的母题。实际委派的 Slide 不读取组外页面，因此边界信息不能只隐含在全册逐页表中；主 Agent 连续制作时直接沿用这些跨页决策；
...
逐页计划是页面制作的内容合同。只有 `## 最终屏显文案` 中通过内容充分性与观众价值检查的内容可以进入 HTML；`页面导演`、`视觉实现`、`Reference route`、`来源` 和 `口语讲稿` 都是非屏显区。屏显文案、数据、节点关系和素材路径必须定稿；制作时可以调整 `.slide-body` 内的比例和排法，并可省略计划中语义完全重复的低价值屏显节点，但不得改事实、自创文案或增加计划外的大型装饰图。`视觉验收` 写观众能从像素读出的结果，不写“做得高级”“有科技感”这类审美愿望；流程/机制/方法/数据/media-led 页需明确语义对象、关系方向与领域证据，纯排印页可用一句话说明焦点与阅读顺序。Box-Agent 静态新建中，`可读性预演` 与 `spatial_budget` 记录字阶、内容优先级和空间意图，不要求逐项预先计算 CSS 高度、padding 或图表坐标，也不另写“预演达标”报告；制作时根据真实文本和素材求解布局。其他环境保留冻结前的版面预演。任何路径发现内容只能靠低于 `--fs-min` 的文字塞下，都应先缩短重复屏显、重组层级；改变页面职责或拆页仍须返回 Story，不能靠缩字交付。

### Reference route 的定位要求

Box-Agent 直接读取原文：写清 reference 文件和实际需要的章节，使 Slide 能唯一定位并完整读取命中原文；可用章节编号、标题或必要的父级标题消歧。示例：`- design-rules.md：4 · 图表(ECharts)`。多个章节应逐项可辨认，不以笼统的“参考设计规范”替代路由；不要求为输入包装逐字匹配 Markdown 标题。按需路由不变，不为包装扫描全库或删减设计要求。

其他环境使用 `group_input.py` 时保留原解析格式：每条用 `- 文件名.md：完整标题`，与实际 Markdown 标题逐字匹配且不含 `#`；重名标题用 `父标题 > 子标题` 消歧，多个章节分条，不用简称、范围或逗号拼接。

Box-Agent 素材路径回填完成后，直接使用原计划、CSS 和参考；主 Agent 可复用当前上下文完整掌握且未变化的原文，实际新委派读取所需输入。组段、Style Lock、逐页计划及命中章节不被 task 中的重述或摘要替换；`boundary_handoff` 保存在 `plan/deck.md` 的所属组。静态新建允许在输入体积合适时一次准备多页输入并连续制作，截断、变更或压缩丢失时补读原文；不以生成 `group_input.py` 分片为前置，已有有效原文分片可复用但不为它迁移计划。其他环境保留素材回填后准备完整原文分片的流程。逐页计划中的 `## 最终屏显文案`、`## 视觉实现` 及 `image_opportunity`、`presentation`、`subject_only` 等既有机读字段仍遵守上面的合同。
