...
- 内容页按连续论证、共享素材或图形语言分组，但一个组还必须共享相容的制作任务。普通 cards/list 可以形成亲缘组；复杂 Canvas、独立数据图、重图像合成等若没有真正共享的构图系统，应各自成组。不要用“都是内容页”替代制作负荷判断；页组规模应适合同一个 Slide 保留设计状态并按页序制作。Box-Agent 在一次委派内写完组内全部首稿，由父级批量渲染、逐页看图并按原组集中反馈；具备子内渲染与看图能力的其他环境保留 Slide 的逐页闭环。两者都覆盖每页像素验收和组内总览；
- 每页只能属于一个组；全部页面必须被覆盖；
...
Box-Agent 素材路径回填完成后，委派直接引用原计划、CSS 和参考；组段、Style Lock、逐页计划及命中章节不被 task 中的重述或摘要替换。`boundary_handoff` 保存在 `plan/deck.md` 的所属组，Slide 从该处读取。先读共享输入，再随页序读当前页计划及命中章节，不以生成 `group_input.py` 分片为新任务前置；已有有效原文分片可以复用，但不为它迁移计划。其他环境保留素材回填后准备完整原文分片的流程。这里取消的只是 Box-Agent 包装所需的标题限制，逐页计划中的 `## 最终屏显文案`、`## 视觉实现` 及 `image_opportunity`、`presentation`、`subject_only` 等既有机读字段仍遵守上面的合同。
